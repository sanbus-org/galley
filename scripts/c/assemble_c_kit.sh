...
#
# One kit serves C and C++ alike (there is no separate C++ binding; both
# consume the same C ABI). There is deliberately no prebuilt library: the
# parser library is per-grammar, generated from the consumer's own .grm,
# so the kit ships the means of production. Consumers still need a Zig
