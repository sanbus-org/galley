...
Dev versions look like 0.1.3-dev.42.gabc12345678 (product, CI run number,
...
Flavors (positional, default: npm):
  npm     npm/cargo semver, used verbatim: 0.1.3-dev.42.gabc12345678
  ...
  maven   Java dev stream, overwritten per push: 0.1.3-SNAPSHOT.
...
Run: python3 scripts/package_version.py [--flavor npm|pep440|maven]
"""
...
def to_maven(version: str) -> str:
    """Map a run version to the Java dev stream (overwritten per push).

    Only the dev suffix is stripped, so a prerelease base keeps its own
    stream: 0.2.0-beta.1-dev.42.gsha becomes 0.2.0-beta.1-SNAPSHOT, never
    the 0.2.0 line's. Versions without a dev suffix pass through
    untouched (stable and prerelease tags publish verbatim).
    """
    dev = re.fullmatch(r"(.*)-dev\..*", version)
    if dev is None:
        return version
    ...
    if not base:
        print(
            f"package_version: cannot derive a maven stream from {version!r}",
            file=sys.stderr,
        )
        raise SystemExit(1)
...
def main() -> None:
    ...
    if len(arguments) == 1:
        ...
        print("package_version: usage: package_version.py [--flavor npm|pep440|maven] [VERSION]",
              file=sys.stderr)
        raise SystemExit(2)
