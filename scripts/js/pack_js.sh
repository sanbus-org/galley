...
#
# The tarballs feed the static registry publisher (dev versions) and the
# stable npmjs publisher (product versions), and double as the
# downloadable record of every build. Staging (copy, compile-kit/CLI
# assembly, version pin) lives in scripts/js/stage_js_package.sh, shared
# with the stable publisher, so both paths stage byte-identical trees for
# the same version. Packing here only runs `npm pack` over them.
#
# Version: ${PACKAGE_VERSION:-run version from scripts/package_version.py}
# (product version on v* tags, dev version otherwise). The repo is never
# touched; dist/ must already be built (npm run build) and CLI_ARTIFACTS
# must hold the compiler binaries for the cli-* packages.
#
