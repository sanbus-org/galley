...
tar --exclude='./build' --exclude='./*.egg-info' --exclude='./__pycache__' --exclude='./tests' --exclude='./test_fixture' -cf - -C "$PACKAGE_DIR" . | tar -xf - -C "$work"
...
"$ROOT/scripts/js/assemble_compile_kit.sh" "$work/galley/compile-kit"
...
for cli_dir in cli-darwin-arm64 cli-darwin-x64 cli-linux-x64 cli-linux-arm64 cli-win32-x64 cli-win32-arm64; do
	mkdir -p "$work/galley/generator/$cli_dir/bin"
	cp "$CLI_ARTIFACTS/$cli_dir"/bin/galley* "$work/galley/generator/$cli_dir/bin/"
	chmod +x "$work"/galley/generator/"$cli_dir"/bin/galley*
done
