...
func main() {
	...
	if hooksPresent {
		...
		if err := emitProcedureShim(filepath.Join(languageDir, "procedures.zig"),
			shimPath); err != nil {
			fatal("%v", err)
		}
...
func emitHookBinding(outputPath string, userHooks []string) error {
	...
	builder.WriteString("// Registers the exported hook addresses from hooks/procedures.go\n")
	builder.WriteString("// into the parser library's procedure slots at package init.\n")
