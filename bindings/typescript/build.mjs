 ...
 *
 * * `procedures.ts` / `procedures.js` — TypeScript/JavaScript hooks
 *   (`export function reduction_<Var>(args)` / `export function hook_<name>(args)`),
 *   dispatched through a generated TypeScript shim. This is the
 *   native-language path mirroring Rust's `procedures.rs`.
 ...
 * through the generic consumer build, and copies libgalley-typescript.* into
 * the language directory so `import { Session } from "galley-typescript"`
...
const LIBRARY_NAME = "galley-typescript";
...
function cacheDir() {
  ...
  const dir = path.join(base, "galley-bindings", "typescript");
...
function findTypeScriptProceduresFile(languageDir) {
  const candidates = [
...
function emitTypeScriptProcedureShim(templatePath, outputPath) {
  let template;
  try {
    template = fs.readFileSync(templatePath, "utf-8");
  } catch (e) {
    fatal(`failed to read ${templatePath}: ${e.message}`);
  }
  ...
  const builder = [];
  builder.push("// Generated by galley-bindings; DO NOT EDIT.");
  builder.push("// Procedure hooks dispatch through a TypeScript callback registered");
  builder.push("// by the host's JS runtime; unregistered slots are no-ops.");
  ...
  builder.push(
    "export fn galley_install_typescript_dispatch(target: *const fn ([*]const u8, usize, ?*anyopaque) callconv(.c) void) void {",
  );
...
function main() {
  ...
  // exist, still generate the TypeScript shim as a no-op fallback so the
  // library links (hooks are simply no-ops until JS registers them via
  // installProcedure), mirroring Python's always-shim model.
  const tsProceduresFile = findTypeScriptProceduresFile(languageDir);
  ...
  if (tsProceduresFile !== null) {
    if (hasCProcedures) {
      console.error(
        `galley-bindings: both TypeScript (${tsProceduresFile}) and C procedures found — using TypeScript`,
      );
    }
    console.error(`galley-bindings: using TypeScript procedures from ${tsProceduresFile}`);
    const shimPath = path.join(languageDir, "procedures_typescript.zig");
    const templatePath = path.join(languageDir, "procedures.zig");
    emitTypeScriptProcedureShim(templatePath, shimPath);
