 ...
 * (`procedures_typescript.zig`, generated by `npx galley-typescript-bindings`)
 ...
 * `currentNode()` plus the ordinary `Node` methods (which call `galley_node_*`
 * on the parsing session).
 */
...
import { copyStringBytes, loadLibrary, toBigInt, type GalleyFFI } from "./ffi.js";
...
export class ProcedureArguments {
  ...
  constructor(args: bigint, session: Session | null, ffi: GalleyFFI) {
    this.#args = args;
  ...
  currentNode(): Node | null {
    ...
    const address = toBigInt(this.#ffi.galley_procedure_current_node(this.#args));
    if (address === INVALID_NODE) return null;
    return new Node(this.#session, address);
  }

  dropSelf(): void {
    const status = this.#ffi.galley_procedure_drop_self(this.#args);
    if ((typeof status === "bigint" ? status : BigInt(status)) < 0n) {
      throw new Error("galley_procedure_drop_self failed");
    }
  }

  dropChildren(): void {
    const status = this.#ffi.galley_procedure_drop_children(this.#args);
    if ((typeof status === "bigint" ? status : BigInt(status)) < 0n) {
  ...
  dropIfEmpty(): void {
    const status = this.#ffi.galley_procedure_drop_if_empty(this.#args);
    if ((typeof status === "bigint" ? status : BigInt(status)) < 0n) {
  ...
  replaceWithChildren(): void {
    const status = this.#ffi.galley_procedure_replace_with_children(this.#args);
    if ((typeof status === "bigint" ? status : BigInt(status)) < 0n) {
  ...
  currentLine(): number {
    return this.#ffi.galley_procedure_context_line(this.#args);
  }
    ...
    const cb = koffi.register((namePtr: unknown, nameLen: unknown, argsPtr: unknown) => {
      ...
      try {
        if (fn.length === 0) {
          ...
          (fn as HookFn)(new ProcedureArguments(argsAddr, currentSession, foreignInterface));
        }
      } catch (err) {
