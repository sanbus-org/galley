...
export async function buildParserArtifact({
  ...
  // Session probe, so it is backend-agnostic. Packaging never fails the
  // artifact build — a skipped entry warns loudly and the directory
  // still opens through Session.fromDirectory.
  const packageName = emitPackageEntry(languageDir);
  if (packageName !== null) {
    console.log(
      `galley-bindings: import ${packageName} from ${path.dirname(languageDir)} or open ${languageDir} with Session.fromDirectory`,
    );
  }
  return destination;
}
 ...
 * (`Session.fromDirectory` works with any name).
 */
 ...
 * the bundled `procedures` scan every `fromDirectory` performs. The
 * universal `Session` stays the only session implementation; the entry
...
function emitPackageEntry(languageDir) {
  ...
  // Check both clobber guards before writing either file: a foreign
    ...
    JSON.stringify(
      {
        ...
        peerDependencies: { "@sanbus/galley": "*" },
  ...
  fs.writeFileSync(
    initPath,
    `${PACKAGE_BANNER}
// Language package: parser sessions plus bundled hook wiring.
import { fileURLToPath } from "node:url";
 ...
 * wire automatically, ahead of explicit \`options.procedures\`.
 *
 * @returns {Promise<Session>}
 */
export function openSession(options = {}) {
  return Session.fromDirectory(LANGUAGE_DIR, options);
}
