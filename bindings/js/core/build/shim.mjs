...
/** Refuses to overwrite an existing file without the marker banner. */
function assertSafeToGenerate(outputPath) {
  if (!lacksGeneratedMarker(outputPath)) return;
  throw new Error(
    `${outputPath} exists and was not generated by galley-js; ` +
      `move it aside or restore its marker banner before rebuilding`,
  );
}

export function emitJsProcedureShim(metadataPath, outputPath) {
  assertSafeToGenerate(outputPath);
  const hooks = readProcedureHooks(metadataPath);

  const builder = [];
  builder.push(GENERATED_BANNER);
