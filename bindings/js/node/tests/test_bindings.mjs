 ...
 * into a temp workdir; sessions open it through languagePath.
 */
...
await test("nested parse across symlinked file keeps hooks", async () => {
  ...
  try {
    ...
    // one shared port, so the inner parse must restore the outer
    // session's gates on unwind. Covers findLibraryFile's half.
    const a = await Session.fromDirectory(languageDir);
    const b = await Session.fromFile(linkFile);
    a.clearProcedures();
    b.clearProcedures();
    let outerCalls = 0;
    let nested = false;
