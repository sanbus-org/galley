...
public final class GalleyBuild {
    ...
    private static void emitParserClass(String packageName, String hookClass, List<String> hooks, Path languageDir) {
        ...
        if (Files.exists(outputPath)) {
            String existing;
            try {
                existing = Files.readString(outputPath, StandardCharsets.UTF_8);
            } catch (IOException e) {
                fatal("failed to read " + outputPath + ": " + e.getMessage());
                return;
            }
            if (!existing.startsWith(GENERATED_BANNER)) {
                fatal(outputPath + " exists and was not generated by galley-bindings; move it aside");
            }
        }
