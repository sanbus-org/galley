...
- The generated `Parser` per grammar wires bundled hooks at package import from the `metadata.json` hook list. Bare `Galley.load` performs no scan, and explicit installs win over bundled ones.
...
- Every named category the shared contracts define is a Java enum with `getCode()`; branching code never hard-codes integers.
