...
pub const cli_options = .{
    ...
    .{ .name = "--with-input-refill", .takes_value = false },
    .{ .name = "--no-input-refill", .takes_value = false },
...
pub const cli_help =
    ...
    \\      --with-input-refill             Enable input-buffer refilling.
    \\      --no-input-refill               Disable input-buffer refilling.
...
pub fn optionsFromCliArgs(args: anytype) Options {
    ...
    for (args) |arg| {
        ...
        } else if (std.mem.eql(u8, arg.name, "--with-input-refill")) {
            result.with_input_refill = true;
        } else if (std.mem.eql(u8, arg.name, "--no-input-refill")) {
            result.with_input_refill = false;
