...
const SAMPLE_PATH = "/tmp/galley-js-wasm-example.json";

function printTree(node: import("@sanbus/galley-wasm").Node, depth: number): void {
  ...
  try {
    ...
    if (args.length > 0) {
      try {
        ...
        const galleyErr = err as import("@sanbus/galley-wasm").GalleyError;
