...
fn addSemanticErrorTests(
    ...
    const generated_name = try std.fmt.allocPrint(b.allocator, "semantic-errors-{s}-{s}-parser.zig", .{ parser_type, mode });
    const generate_parser = b.addRunArtifact(options.generate_parser_file_exe);
    generate_parser.addArg("--grammar");
    ...
    generate_parser.addArg(parser_type);
    generate_parser.addArg("--label");
    generate_parser.addArg(try std.fmt.allocPrint(b.allocator, "{s}/semantic-errors-{s}/tests", .{ parser_type, mode }));
    generate_parser.addArg("--output");
    const generated_parser_path = generate_parser.addOutputFileArg(generated_name);
    generate_parser.addArg("--config-output");
    const generated_config_path = generate_parser.addOutputFileArg(try std.fmt.allocPrint(b.allocator, "semantic-errors-{s}-{s}-config.zig", .{ parser_type, mode }));
    ...
    const parser_name = try std.fmt.allocPrint(b.allocator, "semantic-errors-{s}-{s}", .{ parser_type, mode });
    const generated_parser = common.addGeneratedParserModule(
        b,
        options.target,
        options.optimize,
    ...
    const test_mod = b.createModule(.{
        .root_source_file = b.path("src/tests/semantic_error_test.zig"),
        .target = options.target,
        .optimize = options.optimize,
        .imports = &.{.{ .name = "parser-under-test", .module = generated_parser.runtime_mod }},
    });
...
fn addNoAstTreeHelpersTests(
    b: *std.Build,
    options: Options,
    parser_type: []const u8,
    filters: []const []const u8,
) !*std.Build.Step.Run {
    const parser_name = try std.fmt.allocPrint(b.allocator, "no-ast-tree-helpers-{s}", .{parser_type});
    const generate_parser = b.addRunArtifact(options.generate_parser_file_exe);
    generate_parser.addArg("--grammar");
    generate_parser.addFileArg(b.path("tests/no-ast-tree-helpers/grammar.grm"));
    generate_parser.addArg("--parser-type");
    generate_parser.addArg(parser_type);
    generate_parser.addArg("--label");
    generate_parser.addArg(parser_name);
    ...
    const procedures_mod = b.createModule(.{
        .root_source_file = b.path("tests/no-ast-tree-helpers/procedures.zig"),
        .target = options.target,
        .optimize = options.optimize,
    });
    const config_mod = b.createModule(.{
    ...
    const error_messages_mod = b.createModule(.{
        .root_source_file = b.path("tests/no-ast-tree-helpers/error_messages.zig"),
        .target = options.target,
        .optimize = options.optimize,
    });
    const generated_parser = common.addGeneratedParserModule(
    ...
    const test_mod = b.createModule(.{
        .root_source_file = b.path("src/tests/no_ast_tree_helpers_test.zig"),
        .target = options.target,
        .optimize = options.optimize,
        .imports = &.{.{ .name = "parser-under-test", .module = generated_parser.runtime_mod }},
    });
...
fn addNoAstTerminalTests(
    b: *std.Build,
    options: Options,
    parser_type: []const u8,
    filters: []const []const u8,
) !*std.Build.Step.Run {
    const parser_name = try std.fmt.allocPrint(b.allocator, "no-ast-terminal-{s}", .{parser_type});
    const generate_parser = b.addRunArtifact(options.generate_parser_file_exe);
    generate_parser.addArg("--grammar");
    generate_parser.addFileArg(b.path("tests/no-ast-terminal/grammar.grm"));
    generate_parser.addArg("--parser-type");
    generate_parser.addArg(parser_type);
    generate_parser.addArg("--label");
    generate_parser.addArg(parser_name);
    ...
    const procedures_mod = b.createModule(.{
        .root_source_file = b.path("tests/no-ast-terminal/procedures.zig"),
        .target = options.target,
        .optimize = options.optimize,
    });
    const config_mod = b.createModule(.{
    ...
    const error_messages_mod = b.createModule(.{
        .root_source_file = b.path("tests/no-ast-terminal/error_messages.zig"),
        .target = options.target,
        .optimize = options.optimize,
    });
    const generated_parser = common.addGeneratedParserModule(
    ...
    const test_mod = b.createModule(.{
        .root_source_file = b.path("src/tests/no_ast_terminal_test.zig"),
        .target = options.target,
        .optimize = options.optimize,
        .imports = &.{.{ .name = "parser-under-test", .module = generated_parser.runtime_mod }},
    });
...
fn addNoAstIndentTextTests(
    b: *std.Build,
    options: Options,
    parser_type: []const u8,
    filters: []const []const u8,
) !*std.Build.Step.Run {
    const parser_name = try std.fmt.allocPrint(b.allocator, "no-ast-indent-text-{s}", .{parser_type});
    const generate_parser = b.addRunArtifact(options.generate_parser_file_exe);
    generate_parser.addArg("--grammar");
    generate_parser.addFileArg(b.path("tests/no-ast-indent-text/grammar.grm"));
    generate_parser.addArg("--parser-type");
    generate_parser.addArg(parser_type);
    generate_parser.addArg("--label");
    generate_parser.addArg(parser_name);
    ...
    const procedures_mod = b.createModule(.{
        .root_source_file = b.path("tests/no-ast-indent-text/procedures.zig"),
        .target = options.target,
        .optimize = options.optimize,
    });
    const config_mod = b.createModule(.{
    ...
    const error_messages_mod = b.createModule(.{
        .root_source_file = b.path("tests/no-ast-indent-text/error_messages.zig"),
        .target = options.target,
        .optimize = options.optimize,
    });
    const generated_parser = common.addGeneratedParserModule(
...
fn addVerbatimTests(
    ...
    const mode_name = if (indent) "indent" else "plain";
    const parser_name = try std.fmt.allocPrint(b.allocator, "verbatim-{s}-{s}", .{ mode_name, parser_type });
    const generate_parser = b.addRunArtifact(options.generate_parser_file_exe);
    generate_parser.addArg("--grammar");
    generate_parser.addFileArg(b.path("tests/verbatim/grammar.grm"));
    generate_parser.addArg("--parser-type");
    generate_parser.addArg(parser_type);
    generate_parser.addArg("--label");
    generate_parser.addArg(parser_name);
    ...
    const procedures_mod = b.createModule(.{
        .root_source_file = b.path("tests/verbatim/procedures.zig"),
        .target = options.target,
        .optimize = options.optimize,
    });
    const config_mod = b.createModule(.{
    ...
    const error_messages_mod = b.createModule(.{
        .root_source_file = b.path("tests/verbatim/error_messages.zig"),
        .target = options.target,
        .optimize = options.optimize,
    });
    const generated_parser = common.addGeneratedParserModule(
    ...
    const test_mod = b.createModule(.{
        .root_source_file = b.path(if (indent) "src/tests/verbatim_indent_test.zig" else "src/tests/verbatim_test.zig"),
        .target = options.target,
        .optimize = options.optimize,
        .imports = &.{.{ .name = "parser-under-test", .module = generated_parser.runtime_mod }},
    });
...
fn addVerbatimNullableTests(
    b: *std.Build,
    options: Options,
    parser_type: []const u8,
    filters: []const []const u8,
) !*std.Build.Step.Run {
    const parser_name = try std.fmt.allocPrint(b.allocator, "verbatim-nullable-{s}", .{parser_type});
    const generate_parser = b.addRunArtifact(options.generate_parser_file_exe);
    generate_parser.addArg("--grammar");
    generate_parser.addFileArg(b.path("tests/verbatim-nullable/grammar.grm"));
    generate_parser.addArg("--parser-type");
    generate_parser.addArg(parser_type);
    generate_parser.addArg("--label");
    generate_parser.addArg(parser_name);
    ...
    const procedures_mod = b.createModule(.{
        .root_source_file = b.path("tests/verbatim-nullable/procedures.zig"),
        .target = options.target,
        .optimize = options.optimize,
    });
    const config_mod = b.createModule(.{
    ...
    const error_messages_mod = b.createModule(.{
        .root_source_file = b.path("tests/verbatim-nullable/error_messages.zig"),
        .target = options.target,
        .optimize = options.optimize,
    });
    const generated_parser = common.addGeneratedParserModule(
    ...
    const test_mod = b.createModule(.{
        .root_source_file = b.path("src/tests/verbatim_nullable_test.zig"),
        .target = options.target,
        .optimize = options.optimize,
        .imports = &.{.{ .name = "parser-under-test", .module = generated_parser.runtime_mod }},
    });
